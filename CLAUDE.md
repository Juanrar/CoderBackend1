...
- **Estado (11/08/2026):** el CRUD completo de **productos** ya está de punta a punta y **commiteado**: `GET /api/products`, `GET /api/products/:id`, `POST /api/products`, `PUT /api/products/:id` y `DELETE /api/products/:id` funcionan contra Mongo, con validaciones básicas y manejo de errores (`CastError`, `E11000` duplicado, `ValidationError`) en `producto.controller.js`. **`GET /api/products` quedó 100% alineado con la consigna**: `limit`/`page`/`query`/`sort` funcionando y el formato de respuesta exacto (incluyendo `prevLink`/`nextLink` calculados dinámicamente). La **API de carritos ya arrancó**: modelo con `ref` a producto, dao, controller y router existen y están montados en `/api/carts`; funcionan `POST /api/carts`, `GET /api/carts/:cid` (con `populate`) y `POST /api/carts/:cid/products/:pid` (incrementa cantidad si ya existe). Faltan los 4 endpoints restantes de carrito (`DELETE` de un producto, `PUT` de todos los productos, `PUT` de cantidad de un producto, `DELETE` del carrito completo). Las vistas y WebSockets siguen sin tocarse desde la última auditoría (datos hardcodeados, links rotos, saludo de prueba nomás). Ver sección 7 para la auditoría detallada.
...
- [~] Capa **`controllers`** implementada (decidido usar 3 capas — ver sección 6). → `src/controllers/producto.controller.js` (completo) y `src/controllers/cart.controller.js` (`create/getAll/getById/addProduct`, faltan los métodos de quitar/actualizar/vaciar).
...
> Estado (11/08/2026): **arrancó.** `src/models/cart.model.js`, `src/dao/cart.dao.js`, `src/controllers/cart.controller.js` y `src/routes/cart.router.js` ya existen, montados en `app.js` bajo `/api/carts`. `create`, `getById` (con `populate`) y `addProduct` (con incremento de cantidad) funcionan. Faltan los demás 4 endpoints.
...
- [x] `GET /api/carts/:cid` con **`populate`**. → `CartDao.getPopulatedById` usa `.populate('products.product')`. Separado a propósito de `CartDao.getById` (sin popular), que se usa solo para la lógica interna de `addProduct`.
- [x] `POST /api/carts/:cid/products/:pid` (incrementa cantidad si ya existe). → `CartController.addProduct` busca con `findIndex` sobre el carrito **sin poblar** (necesario para poder comparar `ObjectId` contra `pid`), incrementa `quantity` si ya está o hace `push` si no.
...
└─ src/
   ...
   │  ├─ cart.router.js           /api/carts (POST, GET, GET /:id, POST /:cid/products/:pid)
...
**Todavía no existe:** los 4 endpoints de carrito restantes (`DELETE` producto, `PUT` carrito completo, `PUT` cantidad, `DELETE` carrito completo), la implementación FileSystem, ni `.env`.
...
2. **La API de productos está completa; la de carritos arrancó pero le faltan 4 de 7 endpoints (50% de la nota).** `GET/POST/PUT/DELETE /api/products` (+`:id`) están escritos, commiteados y con manejo de errores razonable, y `GET /api/products` ya cumple el formato exacto de la consigna. De carritos ya funcionan `POST /api/carts`, `GET /api/carts/:cid` (con `populate`) y `POST /api/carts/:cid/products/:pid`. Faltan `DELETE /api/carts/:cid/products/:pid`, `PUT /api/carts/:cid`, `PUT /api/carts/:cid/products/:pid` y `DELETE /api/carts/:cid`.
